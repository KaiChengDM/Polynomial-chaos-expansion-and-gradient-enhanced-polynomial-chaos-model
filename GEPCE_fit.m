function PCEmodel = GEPCE_fit(input,output,grad_output,par)

% Training a GE-PCE model with Gaussian process regression
...
grad_output = reshape((grad_output.*(ub-lb)/std_output/2),n*m,1);      % Transform the gradient data
...
PCEmodel.grad_output = grad_output;

PCEmodel.sample_size = m;
PCEmodel.dim = n;

[measure_mat num trunc_index] = GE_measurement(u,order,polytype,q);  % Generate measurement matrix of GE-PCE basis function

PCEmodel.measure_mat = measure_mat;
PCEmodel.basisnumber = num;
PCEmodel.basisindex = trunc_index;

Sparsemodel = GEPCE_train(PCEmodel);   % Training sparse GE-PCE 
