function model = GEPCE_train(model)

% Training GE-PCE 

%% Preparation

m = model.sample_size; n = model.dim;  m1 =  m*(n+1); measure_mat = model.measure_mat;

output = model.output; grad_output = model.grad_output;

total_output = [output; grad_output];      % Model responses and its gradients
...
for i = 1 : order
   P(i) = length(find(Order<=i));  % The number of the terms in covariance function
end
...
%   [R1 rd] = chol(Cov +10^-7.*I); 
 ...
 ind = find( w >10^-20);    % find the significant terms
 ...
 sigma2 = ((total_output-current_mat*Omega)'*(total_output-current_mat*Omega))/(m1-sum(gamma)); % Update of noise variance 
  
 if (size(Omega)==size(Omega_old))
      dmu = max(abs(Omega_old-Omega)); % Convergence threshold
