function [Y_pre V_pre] = PCE_predict(x_pre,model)
...
% Model parameter
 ...
 u_pre = 2.*(x_pre-repmat(lb_input,m1,1))./(repmat(ub_input,m1,1)-repmat(lb_input,m1,1))-1;   % Normalization of input data
  
 num = size(measure_mat,2);
  
 measure_pre = Measurement(u_pre,order,polytype,truncation); % Generate measurement matrix of PCE basis function

 measure_pre = measure_pre(:,sparse_basis); 

 covcoef = model.covcoef;

 coef = model.coef; % PCE coefficients
